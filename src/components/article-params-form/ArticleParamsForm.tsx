...
export const ArticleParamsForm: FC<ArticleParamsFormProps> = ({articleState, setArticleState}) => {
	...
	return (
		<div ref={ref}>
			<ArrowButton isActive={isOpen}
						 onClick={handleButtonClick}/>
			<aside className={clsx(styles.container, {
				[styles.container_open]: isOpen
			})}>
				<form className={styles.form} onSubmit={handleFormSubmit}>
					<Text as="h2" weight={800} uppercase size={31}>Задайте параметры</Text>
					<Select title="шрифт"
							selected={articleFormState.fontFamilyOption}
							options={fontFamilyOptions}
							onChange={option => handleFormElementChange(option, 'fontFamilyOption')}/>
					<RadioGroup title="Размер шрифта"
								name="Размер шрифта"
								options={fontSizeOptions}
								selected={articleFormState.fontSizeOption}
								onChange={option => handleFormElementChange(option, "fontSizeOption")}/>
					...
					<Select title="Цвет фона"
							...
							onChange={option => handleFormElementChange(option, 'backgroundColor')}/>
					<Select title="Ширина контента"
							selected={articleFormState.contentWidth}
							options={contentWidthArr}
							onChange={option => handleFormElementChange(option, 'contentWidth')}/>
